...
//            $getPrincipalInterest = DB::table('tbl_sys_settings')
//                ->where('fieldcode','PAYOUTINTEREST')
//                ->first();
//            if ( $getPrincipalInterest ) {
...
//        dd($password);
    ...
    public function loadDashboard()
    {
        ...
        $getTodayPayoutsAmount = DB::table('tbl_user_payouts')
                ->where('released_date','=',DB::raw('DATE(now())'))
                ->where('status','1')
                ->selectRaw("ROUND(SUM(daily_growth_amount),2) as totalPayoutAmount,COUNT(DISTINCT payout_date) AS totalPayoutCount")
                ->first();
